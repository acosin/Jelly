...
public class Square : MonoBehaviour
{
    ...
    public Vector3 gravity;//重力，即掉落方向和速度
    ...
    public Item GenItem(bool falling = true)
    {
        ...
        if (falling)
        {
            item.transform.position = transform.position + Vector3.back * 0.2f + gravity * -3f;
    ...
    {
        int beginMineRow = LevelManager.THIS.maxRows - LevelManager.THIS.mineRows;
        if (((row <= beginMineRow && gravity.y<0) || (row == LevelManager.THIS.maxRows-1 && gravity.y>0)) && !safe)
            return null;
        return LevelManager.THIS.GetSquare(col, row + (int)gravity.y, safe);
    }

    public Square GetNeighborBottom(bool safe = false)
    {
        int beginMineRow = LevelManager.THIS.maxRows - LevelManager.THIS.mineRows;
        if ((row >= LevelManager.THIS.maxRows && gravity.y<0 || row<= beginMineRow && gravity.y>0)&& !safe)
            return null;
        return LevelManager.THIS.GetSquare(col, row - (int)gravity.y, safe);
    }
        ...
        if (item != null)
        {
            ...
            if (nextSquare != null)
            {
                if(gravity.y < 0)
                {
                    ...
                    {
                        int beginMineRow = LevelManager.THIS.maxRows - LevelManager.THIS.mineRows;
                        for (int i = row - 1; i > beginMineRow; i--)
                        {
