...
public class CombineManager
{
    ...
    public List<List<Item>> GetCombineItems()
    {
        //考虑pve,只消除玩家，怪物不需要计算消除；pvp才需要考虑对手部分，但是对手也是由服务器发送来的。因此，不必考虑
...
//		Debug.Log (combine.hCount + " " + combine.vCount);
	...
	{
		combine.AddingItem = item;//赋值的时候，同时计算hCount和vCount
		dic [item] = combine;
...
public class Combine
{
	...
	{
		if (items.Count > 0) {
            if (item.square.row != PreviousItem ().square.row)//不相等即可,不必大于
				return true;
		} else
			return true;
