...
public class LevelMakerEditor : EditorWindow
{
	...
	int maxRows;
	int maxCols;
    int opponentRows;//对手所占的行数
    int spaceRows;//敌我隔离区域占的行数
    int mineRows;//自己所占的行数 maxRows = opponentRows + spaceRows + mineRows;
...
//            GooglePlayServices.PlayServicesResolver.MenuResolve();//2.1.2
	...
	void GUILevelSize()
	{
        ...
        int oldValue2 = opponentRows + mineRows;
        ...
        opponentRows = EditorGUILayout.IntField("opponentRows", opponentRows, new GUILayoutOption[] {
            GUILayout.Width (50),
            GUILayout.MaxWidth (200)
        });

        spaceRows = EditorGUILayout.IntField("spaceRows", spaceRows, new GUILayoutOption[] {
            GUILayout.Width (50),
            GUILayout.MaxWidth (200)
        });
        mineRows = maxRows - spaceRows - opponentRows;
        EditorGUILayout.IntField("mineRows", mineRows, new GUILayoutOption[] {
            GUILayout.Width (50),
            GUILayout.MaxWidth (200)
        });
		...
		if (maxCols > 9)
			maxCols = 9;
        if (oldValue != maxRows + maxCols || oldValue2 != opponentRows + mineRows)
		{
			Initialize();
			SaveLevel();
		}
	...
	public void SaveMap(string fileName)
	{
		...
		saveString += "\r\n";
        saveString += "SIZE " + maxCols + "/" + maxRows + "/" + opponentRows + "/" + spaceRows;
		saveString += "\r\n";
